...
<script type="text/javascript">
		function addOrder(sum) {
			location.href = "order?action=createOrder&aid="+$("#address").val()+"&sum="+sum;
		}
</script>
</head>
...
<div class="container" style="background-color: white;">
 				 	...
 				 	<select id="address" style="width:60%" class="form-control">
 				 		<c:forEach items="${addList}" var="a" varStatus="ai">
							<c:if test="${a.a_state == 1}">
								<option selected value="${a.a_id}">${a.a_name}&nbsp;&nbsp;${a.a_phone}&nbsp;&nbsp;${a.a_detail}</option>
							</c:if>
	...
	<div class="row pull-right" style="margin-right: 40px;">
		 <div style="margin-bottom: 20px;">
	            <button  onclick="addOrder(${sum})" id="btn_add" class="btn  btn-danger btn-lg" type="button">提交订单</button>
	     </div>
	</div>
</div>
