...
public class Test1 {
	public static void main(String[] args) {
        ...
        //左边找到Outer类里面的Inner类 = 右边是实例Outer类对象再去实例Inner类（静态内部类就可以.内部类名）
...
//调用方式的局限性；局部变量如果在局部内部类中使用了，则自动+final
class Outter3{
	String name = "张三";
	public void show() {
		int age = 30;
...
概述与特点，与抽象类的异同
2.应用场景（重点）
接口直接引用多态；接口与抽象类实现多态的综合案例；接口与类之间的关系
接口传参多态；接口与抽象类实现多态的综合案例；多态的好处
3.常量接口
状态值放到接口中使用；返回值多态中使用接口常量
4.内部类
概述； 特点；分类
成员内部类-调用方式，成员内部类中不能定义静态属性
静态内部类-调用方式，静态内部类中不能使用成员变量
局部内部类-调用方式的局限性；局部变量如果在局部内部类中使用了，则自动+final
```
