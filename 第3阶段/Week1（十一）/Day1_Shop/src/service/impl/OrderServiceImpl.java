...
public class OrderServiceImpl implements OrderService {
    ...
    public int createOrder(String aid, String sum, int uId) {
        ...
        try {
            ...
            for (Cart c : carts) {
                Item it = new Item(0,oid,c.getP_id(),c.getC_count(),c.getC_num(),null);
                items.add(it);//存入订单详情
            }
    ...
    public Orders showDetail(String oid) {
        ...
        try {
            //1.根据oid获取到订单对象
            orders = orderDao.selectOrdersByOid(oid);
            //2.根据oid获取到订单详情集合
            List<Item> list = orderDao.selectIteamsByOid(oid);
            //3.订单对象中设置详情集合
            orders.setList(list);
    ...
    @Override
    public int updateOidState(String oid) {
        try {
            return orderDao.updateOidState(oid);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
